...
class CourseController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Query to get available prestations
        ...
        // Transform prestations with calculated values
        ...
        return view('course', [
            ...
            'city' => $city,
            'polyline' => $polyline,
            'startCoords' => $startCoords, // Координаты начальной точки
            'endCoords' => $endCoords
        ]);
    }
    public function getDistanceFromApi($startAddress, $endAddress)
{
    ...
    // Формирование URL с координатами
    ...
    try {
        ...
        if (isset($data['paths'][0]['distance'])) {
            return number_format($data['paths'][0]['distance'] / 1000, 1); // Возвращаем с одной цифрой после точки
        }
    } catch (\Exception $e) {
...
{
    $client = new Client();
    $apiKey = 'a2404e3a-1aef-4546-a2e8-7477f836a79d'; // Ваш API ключ

    // Получение координат для начальной и конечной точек
    $startCoords = $this->getCoordinatesFromAddress($startAddress);
    $endCoords = $this->getCoordinatesFromAddress($endAddress);
    ...
    try {
        ...
        if (isset($data['paths'][0]['time'])) {
            // Время в пути возвращается в миллисекундах, переводим в минуты
            $timeInMinutes = $data['paths'][0]['time'] / 60000;
            return round($timeInMinutes, 1); // Возвращаем время, округлённое до 1 знака после запятой
        }
    } catch (\Exception $e) {
        error_log("Erreur lors de l'appel à l'API GraphHopper : " . $e->getMessage());
    }
...
public function decodePolyline($encoded)
{
    ...
    while ($index < $len) {
        ...
        // Converting to latitude and longitude in decimal degrees
    ...
    public function detailCourse(Request $request)
    {
        ...
        $course = (object)[
            ...
            'tripTime' => $request->input('tripTime'),
            'tripDate' => $request->input('tripDate'),
            'adjusted_time' => $request->input('adjusted_time'),
            'calculated_price' => $request->input('calculated_price'),
            'distance' => $request->input('distance'),
            'libelleprestation' => $request->input('libelleprestation'),
            'idprestation' => $request->input('idprestation'),
            'descriptionprestation' => $request->input('descriptionprestation'),
            'imageprestation' => $request->input('imageprestation'),
        ];
        $newCourse = false;
    ...
    public function validateCourse(Request $request, $id)
    {
        ...
        if ($course->status !== 'completed') {
            return response()->json(['message' => 'La course doit être terminée pour être validée.'], 400);
        }

        $course->is_validated = true;
        $course->save();

        return response()->json(['message' => 'Course validée avec succès.'], 200);
    }
    ...
    public function CourseAdd(Request $request)
    {
        ...
        $courseJson = $request->input('course');

        $course = json_decode($courseJson);

        $tripDate = Carbon::createFromFormat('d-m-Y', $course->tripDate)->format('Y-m-d');

        $tripDate_r = Carbon::now()->format('Y-m-d');

        $tripTime_c = Carbon::now('Europe/Paris')->format('H:i:s');
        ...
        $adresse_debut = Adresse::where('libelleadresse', $course->startAddress)
            ->select('idadresse')
            ->first();

        $adresse_fin = Adresse::where('libelleadresse', $course->endAddress)
            ->select('idadresse')
            ->first();
        ...
        Reservation::create([
            ...
            'idplanning' => 1,
            'pourqui' => 'moi',
            'datereservation' => $tripDate_r,
            'heurereservation' => $tripTime_c
        ]);


        $maxIdReservation = Reservation::max('idreservation');
         $newIdreservation =Course::create([
            'idreservation' => $maxIdReservation,
            'idprestation' => $course->idprestation,
            'prixcourse' => $course->calculated_price,
            'distance' => $course->distance,
            'idadresse' => $adresse_debut->idadresse,  // Utilise l'ID de l'adresse de début
            'adr_idadresse' => $adresse_fin->idadresse,  // Utilise l'ID de l'adresse de fin
        ...
        return view('detail-course', [
            'course' => $course,
            'newCourse' => $newCourse,
            'newIdreservation' => $newIdreservation
     ...
     public function rateAndInvoice(Request $request)
     {

        DB::table('course')
        ->where('idreservation', $request->idreservation)
