document.addEventListener('DOMContentLoaded', function () {
    ...
    const descriptionParagraph = document.getElementById('cookie-description');

    function updateDescription() {
        if (statisticsCheckbox.checked) {
            descriptionParagraph.textContent = "Les cookies d'analyse permettent à Uber d'analyser vos visites et actions sur ses applications et sites Web ainsi que ceux d'entreprises tierces, afin de mieux comprendre vos centres d'intérêt et de vous proposer des annonces plus pertinentes sur d'autres applications et sites Web.";
        } else if (advertisingCheckbox.checked) {
            descriptionParagraph.textContent = "Les cookies de ciblage publicitaire permettent à Uber de partager vos données avec des partenaires publicitaires, y compris des services de réseaux sociaux, et ce afin de vous envoyer des publicités plus pertinentes via d'autres applications et sites Web, et aux fins déterminées par ces partenaires.";
        } else {
            descriptionParagraph.textContent = "Les cookies essentiels sont nécessaires aux fonctionnalités fondamentales de notre site ou de nos services, telles que la connexion au compte, l'authentification et la sécurité du site.";
        }
    }
    function hideBanner() {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function checkCookieConsent() {
        try {
            const userIP = await getIPAddress();
            const consentGiven = localStorage.getItem(`user_cookie_consent_${pageURL}_${userIP}`);  // Nouvelle clé
            if (consentGiven) {
                cookieBanner.classList.add("hidden");
                settingsBanner.style.display = "none";
            } else {
    ...
    acceptButtonMain.addEventListener("click", () => handleCookieConsent("accepted"));
    rejectButtonMain.addEventListener("click", () => handleCookieConsent("rejected"));

    acceptButtonSettings.addEventListener("click", () => handleCookieConsent("accepted"));
    rejectButtonSettings.addEventListener("click", () => handleCookieConsent("rejected"));
